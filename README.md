...
- **语音编辑**：先选中文本，再按住 `⌘ + Option + .` 说出指令（如“改正式”“翻译成英文”），自动替换选区。
- **AI 口语整理**：自动去除「嗯、啊、那个」等填充词，修正改口，自动分段加标点。
- **个人词典**：自定义听错词 → 正确词映射，识别后、润色前自动替换。
...
- **本地历史与统计**：保存最近 200 条转写，支持复制、删除、导出 Markdown，分别统计录音/编辑的累计/今日字数。
- **轻量设置界面**：配置 API Key、模型、录音模式、自动回车、剪贴板阈值、个人词典等。
...
3. 需要编辑已有文本时，先选中目标文字，再按住 `⌘ + Option + .` 说出指令（如“改正式”“翻译成英文”），松开自动替换选区。
4. 说话，松开/结束录音后 1~3 秒内文字会自动出现在光标处。
5. 打开主窗口可查看历史记录、统计、个人词典和设置。
...
| `⌘ + Option + .` | 语音编辑：先选中文本，按住说出指令，松开自动替换选区 |
| `Esc` | 取消当前录音 |
