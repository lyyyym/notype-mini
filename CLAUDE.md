...
编辑模式需要捕获目标应用的选中文本（`Cmd+C`）并把结果输入回同一位置。如果在此期间显示录音气泡窗口，即使气泡不调用 `set_focus`，macOS 也可能把焦点交给气泡或导致焦点状态不稳定，最终编辑结果输入到了错误的应用或位置。

**正确做法：** 编辑模式不显示录音气泡，只通过状态事件/托盘图标提供反馈，确保目标应用全程保持焦点。转写模式仍保留气泡作为视觉反馈。

### 编辑模式要显式还原目标应用焦点

即使隐藏了气泡，macOS 仍可能在 LLM 处理期间把焦点切回 NoType Mini 主窗口（例如用户点击、系统事件）。日志显示捕获选区时前置应用是 `Notes`，输入前却变成了 `notype-mini`，结果文字被输入到 NoType Mini 自身。

**正确做法：** 在捕获选区时记录目标应用名称，输入编辑结果前通过 `osascript`/`System Events` 显式将该应用重新置为 frontmost，确保文字回到原位置。
